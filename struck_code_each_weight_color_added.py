...
# set weight, lwh
WEIGHT = [ [ random.randint(1, 20) for j in reversed(range(test[i]))] for i in range(TOP_CLASS)]
LWH = [[[random.randint(3, 7), random.randint(3, 7), random.randint(3, 7)] for j in range(test[i])] for i in range(TOP_CLASS)]
...
for TEST in range(TOP_CLASS):
    ...
    LWH = [[[random.randint(3, 5), random.randint(3, 5), random.randint(3, 5)] for j in range(BOX_NUM[i])] for i in range(CLASS)]
